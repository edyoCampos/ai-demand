...
/**
 * NormalizationEngine - Inspirado no DemandAI Core.
 */
...
export function normalizeToolArguments(
  ...
  try {
      ...
      if (inputSchema && inputSchema.shape) {
        ...
        for (const actual of actualFields) {
          if (!expectedFields.includes(actual)) {
            const target = expectedFields.find(f => 
              f.startsWith(actual) || 
              actual.startsWith(f) || 
    ...
    if (inputSchema && inputSchema.shape) {
      ...
      if (fields.length === 1) return { [fields[0]]: parsed };
    }
 ...
 */
export function normalizeArguments(args: string | any, schema?: any): any {
  return normalizeToolArguments('', typeof args === 'string' ? args : JSON.stringify(args), schema);
}

/**
 * Normaliza mensagens para o formato aceito pelas APIs de chat (DemandAI Standard).
